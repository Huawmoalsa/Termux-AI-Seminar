...
class SeminarConsole:
    ...
    def agent_response(
        ...
        if not text or text == "[EMPTY RESPONSE]":
            ...
            or text.startswith("[External delegation unavailable]")
        ):
            status = "(خطأ)"
        else:
            status = f"{len(text.split())} words"
...
class SeminarManager:
    ...
    def collect_proposals(
        ...
        # Reserve budgets before launching parallel work.
        eligible: List[str] = []
        ...
        # Independent proposal agents run concurrently.
        with ThreadPoolExecutor(
            max_workers=max(1, len(eligible))
        ) as pool:
            results = list(
                pool.map(run_proposal, eligible)
            )

        # Merge results sequentially into Seminar state.
        for (
            agent_id,
            content,
            error,
    ...
    def _develop_independently(
        ...
        for agent_id in eligible:
            if self._consume_budget(agent_id):
                reserved.append(agent_id)

        def run_development(agent_id: str):
            agent = self._agent(agent_id)
            started_at = time.perf_counter()
        ...
        # Independent development agents run concurrently.
        with ThreadPoolExecutor(
            max_workers=max(1, len(reserved))
        ) as pool:
            results = list(
                pool.map(run_development, reserved)
            )

        variants: List[Candidate] = []

        # Merge all worker results into Seminar state
        # only after the parallel stage has completed.
        for (
            agent_id,
            content,
            error,
...
class _ThreadLocalStdout:
    """
    Process-wide stdout proxy with per-thread suppression.

    Normal/main-thread output goes to the original stdout.
    LLM worker threads can suppress their internal renderer output
    without changing sys.stdout globally for other threads.
    """
...
_SEMINAR_STDOUT = _ThreadLocalStdout(sys.stdout)
sys.stdout = _SEMINAR_STDOUT


@contextmanager
def _suppress_thread_stdout():
    previous = getattr(
        _SEMINAR_STDOUT._local,
        "suppressed",
        False,
    )

    _SEMINAR_STDOUT._local.suppressed = True

    try:
        yield
    finally:
        _SEMINAR_STDOUT._local.suppressed = previous
...
def main() -> int:
    ...
    Important:
        --llm does NOT execute Android actions.
    ...
    if args and args[0] in {"--demo", "demo"}:
        ...
        print("Android execution is not enabled by this CLI.")
        return 0
    ...
    return 0 if state.phase == SeminarPhase.COMPLETED else 1
