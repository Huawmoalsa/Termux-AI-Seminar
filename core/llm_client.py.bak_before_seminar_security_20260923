...
# Temporary server/provider failures are retried only a small number
# of times on the same key/model. They must never consume the generic
# error retry budget.
_TRANSIENT_RETRY_LIMIT = 2
_TRANSIENT_RETRY_DELAY = 1.0

# Unknown/unclassified errors are bounded as well.
_UNKNOWN_ERROR_RETRY_LIMIT = 3
_UNKNOWN_ERROR_RETRY_DELAY = 1.0
...
def _available_key_count(provider_id: str, model_name: str) -> int:
    ...
    return sum(
        ...
        for key in keys
        if key not in bad
        and _rate_limited_until.get(
            (provider_id, key, model_name), 0.0
        ) <= now
    )
...
def _temporary_retry_delay() -> float | None:
    ...
    deadlines = [
        until
        for until in (
            *_rate_limited_until.values(),
            *_transient_until.values(),
        )
        if until > now
    ]
...
def _reset_provider(provider_id: str) -> None:
    # Compatibility helper: reset temporary provider state.
    # Permanent failures stay disabled, except account restrictions which
    # are deliberately cooldown-based.
    _account_restricted_until.pop(provider_id, None)
...
def _msg_to_dict(msg) -> dict:
            ...
            for tc in tool_calls:
                ...
                if extra is not None:
                    if isinstance(extra, dict):
                        ...
                        tc_dict["extra_content"] = extra.model_dump(
                            exclude_none=True
                        )
    ...
    if "reasoning_details" not in d and hasattr(
        msg,
        "model_extra",
    ):
        extra = getattr(
            ...
            for i, tc in enumerate(m["tool_calls"]):
                ...
                if going_to_gemini:
                    if has_sig:
                        ...
                        clean_tc["extra_content"] = {
                            "google": {
                                "thought_signature": _DUMMY_SIG
                            }
                        }
...
# off. Expanded mode is never capped.
...
def _tool_header_lines(
    ...
    """Build presentation-only tool header lines."""
    tag = _TOOL_TAGS.get(
        name,
        name.upper(),
    )
    ...
    if name == "run_code":
        ...
        if expanded:
            ...
            preview_lines = [
                (
                    l
                    if len(l) <= _COLLAPSED_LINE_CHAR_CAP
                    else l[
                        : _COLLAPSED_LINE_CHAR_CAP - 3
                    ] + "..."
                )
    ...
    if name in ("write_file", "read_file"):
        ...
        if name == "write_file":
            ...
            return [
                f"[{tag}] {path} | mode: {mode} | "
                f"{unit}: {span}"
            ]

        return [
            f"[{tag}] {path} | {unit}: {span}"
        ]
    ...
    if name == "find_replace":
        ...
        find_preview = (
            find_text[:80]
            + ("..." if len(find_text) > 80 else "")
        )

        find_preview = find_preview.replace(
            chr(10),
            "\n",
        )
        ...
        if (
            ...
            seg_desc = (
                f" lines {segment_start or 1}..{end_desc}"
            )
...
def _dispatch_tool(
    ...
    routes = {
        ...
        "get_pending_whatsapp_messages": lambda:
        ...
        "fetch_whatsapp_chat_history": lambda:
            fetch_whatsapp_chat_history(
                to_phone=g("to_phone", ""),
                limit=int(g("limit", 5)),
            ),

        "set_whatsapp_busy_mode": lambda:
            set_whatsapp_busy_mode(
                ...
                exclude_all_groups_except=g(
                    "exclude_all_groups_except",
                    None,
                ),
            ),

        "get_whatsapp_report": lambda:
            get_whatsapp_report(
                clear=_safe_bool(
                    g("clear", False)
                ),
        ...
        "set_whatsapp_user_profile": lambda:
        ...
        "run_diagnosis": lambda:
            run_diagnosis(),

        "silence_whatsapp_contact": lambda:
        ...
        "react_to_whatsapp_message": lambda:
        ...
        "get_whatsapp_contact_info": lambda:
            get_whatsapp_contact_info(
                jid=g("jid", ""),
            ),

        "get_whatsapp_group_participants": lambda:
            get_whatsapp_group_participants(
                jid=g("jid", ""),
            ),

        "download_whatsapp_media": lambda:
        ...
        "schedule_whatsapp_message": lambda:
        ...
        "set_whatsapp_seen": lambda:
            set_whatsapp_seen(
                jid=g("jid", ""),
            ),

        "delegate_subtask": lambda:
            delegate_subtask(
                task=g("task", ""),
                context=g("context", ""),
                model=g(
    ...
    if fn:
        ...
        header_lines = _tool_header_lines(
            name,
            args,
            expanded,
        )
...
def _display_reasoning(
    ...
    print(
        f"\n{GRAY}[Reasoning | "
        f"{provider_id}/{model_name}]{RESET}"
    )
    ...
    if display_state.is_expanded():
        ...
        print(
            f"{GRAY}[reasoning details collapsed: "
            f"{words} words] "
            f"Press Ctrl+O to expand.{RESET}"
        )

    print(
        f"{GRAY}[End reasoning]{RESET}\n"
    )
...
def _ask_with_slots(
    ...
    # API error tracking.
    # Transient 5xx errors use their own small retry budget.
    # Unknown errors use a separate bounded budget.
    consecutive_error_identity: tuple[
        str,
        str,
        str,
    ] | None = None
    ...
    transient_retries: dict[
        tuple[str, str, str],
        int,
    ] = {}

    unknown_error_retries: dict[
        tuple[str, str, str],
        int,
    ] = {}

    # Track which model/provider the current in-memory history belongs to.
    history_model_identity: tuple[
        str,
        str,
    ] | None = None
    ...
    reasoning_exhaustion_retries: dict[
        tuple[str, str, str],
        int,
    ] = {}

    # Invalid request recovery is scoped to provider+model.
    invalid_request_retries: set[
        tuple[str, str]
    ] = set()
    ...
    while True:
        ...
        # IMPORTANT:
        # A transient 5xx cooldown is provider+model scoped.
        # Do not simply choose another key and keep hammering the
        # same failing endpoint.
        if (
            _transient_until.get(
                (pid, model_name),
                0.0,
            )
            > time.monotonic()
            ...
            print(
                ...
                f"Temporary provider cooldown active. "
                f"Skipping this model for the current turn.{RESET}"
            )
        ...
        if forced_retry_key is not None:
            retry_pid, retry_model, retry_key = (
                forced_retry_key
            )

            if (
                retry_pid == pid
                and retry_model == model_name
            ):
                api_key = retry_key
        ...
        if slot != last_slot:
            ...
            carried = max(
                0,
                len(messages) - len(base_messages),
            )
            ...
            if carried > 0:
                print(
                    f"{YELLOW}[{pid}/{model_name}] "
                    f"Continuing with "
                    f"{carried} accumulated "
                    f"message(s) from previous model."
                    f"{RESET}"
                )
        ...
        try:
            while True:
                ...
                # Successful response resets API error state.
                consecutive_error_identity = None
                consecutive_error_signature = None
                consecutive_error_count = 0
                forced_retry_key = None

                transient_retries.pop(
                    (
                        pid,
                        model_name,
                        api_key,
                ...
                if msg_dict.get("tool_calls"):
                    ...
                    for tc_dict, raw_tc in zip(
                        msg_dict["tool_calls"],
                        raw_tcs,
                    ):
                        raw_extra = raw_tc.get(
                            "extra_content"
                        )
                ...
                if msg_dict.get("tool_calls"):
                    if finish_reason == "length":
                        print(
                            f"{RED}[{pid}/{model_name}] "
                            f"Tool call truncated. "
                            ...
                            messages.append(
                                {
                                    ...
                                    "content": (
                                        "[ERROR] Tool call was "
                                        "cut off by the token "
                                        "limit. Please retry "
                                        "with smaller or fewer "
                                        "arguments."
                                    ),
                                }
                            )
                        ...
                        if is_failure:
                            ...
                            if count == 2:
                                tool_result += (
                                    ...
                                    "call. Use a different "
                                    "tool/command or explain "
                                    "the blocker."
                                )
                ...
                if (
                    ...
                    retry_count = (
                        reasoning_exhaustion_retries.get(
                            exhaustion_key,
                            0,
                        )
                    )
                    ...
                    if retry_count == 0:
                        ...
                        print(
                            ...
                            f"exhausted before a "
                            f"final answer. "
                            f"Falling back instead "
                            f"of continuing the "
                            f"same reasoning loop."
                            f"{RESET}"
                        )
            ...
            # ----------------------------------------------------
            # 1. Context/token errors
            # ----------------------------------------------------
            # These are recoverable by reducing max_tokens.
            # This check intentionally comes before generic 400
            # handling because context errors may be HTTP 400.
            if (
                _is_context_error(exc)
                and max_tok
                and max_tok > 512
            ):
                slot_cfg["max_tokens"] = (
                ...
                print(
                    f"{YELLOW}[{pid}/{model_name}] "
                    f"Context/token limit hit. "
                    f"Reducing max_tokens to "
                    f"{slot_cfg['max_tokens']} "
            ...
            # ----------------------------------------------------
            # 2. Account / organization restriction
            # ----------------------------------------------------
            if _is_account_restriction(exc):
                print(
                    f"{RED}[{pid}/{model_name}] "
                    f"Account/organization restriction "
                    f"detected (400). "
                    f"Skipping provider for the cooldown "
                    f"period.{RESET}"
                )
            ...
            if _is_rate_limit(exc):
                print(
                    f"{YELLOW}[{pid}/{model_name}] "
                    f"Rate limited (429). "
                    f"Skipping current key and "
                    f"continuing fallback.{RESET}"
                )
            ...
            # ----------------------------------------------------
            # 5. Permission rejection 403
            # ----------------------------------------------------
            if status == 403:
                print(
                    f"{YELLOW}[{pid}/{model_name}] "
                    f"Permission rejected (403). "
                    f"Disabling current key/path and "
                    f"continuing fallback.{RESET}"
                )
            ...
            # ----------------------------------------------------
            # 6. Model/endpoint not found 404
            # ----------------------------------------------------
            if status == 404:
                print(
                    f"{YELLOW}[{pid}/{model_name}] "
                    f"Model/endpoint not found (404). "
                    f"Disabling this model and continuing "
                    f"fallback.{RESET}"
                )
            ...
            # ----------------------------------------------------
            # 7. Explicit invalid request 400/422
            # ----------------------------------------------------
            if (
                _is_invalid_request(exc)
                ...
                if model_identity not in invalid_request_retries:
                    invalid_request_retries.add(
                        model_identity
                    )
                ...
                print(
                    ...
                    f"Not retrying the same request repeatedly; "
                    f"skipping this model/path.{RESET}"
                )
            ...
            # ----------------------------------------------------
            # 8. Transient server/provider failures 5xx
            # ----------------------------------------------------
            if _is_transient(exc):
                retry_count = transient_retries.get(
                    current_identity,
                    0,
                )
                ...
                if broadcast_mode:
                    ...
                    print(
                        f"{YELLOW}[{pid}/{model_name}] "
                        f"Transient provider failure "
                        f"(status {status or 'unknown'}) "
                        f"in broadcast mode. "
                        f"Returning immediately to preserve "
                        f"broadcast latency.{RESET}"
                    )

                    return (
                        f"[ERROR] Provider "
                        f"{pid}/{model_name} unavailable "
                        f"for broadcast "
                        f"(status {status or 'unknown'})."
                    )

                if retry_count < _TRANSIENT_RETRY_LIMIT:
                    retry_count += 1
                    ...
                    print(
                        ...
                        f"(status {status or 'unknown'}), "
                        f"retry {retry_count}/"
                        f"{_TRANSIENT_RETRY_LIMIT} "
                        f"on current key.{RESET}"
                    )
                ...
                print(
                    f"{RED}[{pid}/{model_name}] "
                    f"Transient error reached "
                    f"{_TRANSIENT_RETRY_LIMIT} retries. "
                    f"Cooling down this provider/model "
                    f"and moving to fallback.{RESET}"
                )
            ...
            if (
                consecutive_error_identity
                == current_identity
                and consecutive_error_signature
                == error_signature
            ):
            ...
            print(
                ...
                f"API/runtime error "
                f"(status {status or 'unknown'}), "
                f"attempt {unknown_count}/"
                f"{_UNKNOWN_ERROR_RETRY_LIMIT} "
                f"on current key.{RESET}"
            )
            ...
            print(
                f"{RED}[{pid}/{model_name}] "
                f"Unknown error reached "
                f"{_UNKNOWN_ERROR_RETRY_LIMIT} "
                f"attempts. Changing API key/path."
                f"{RESET}"
            )
    ...
    if _broadcast_requested(
        ...
        try:
            ...
            print(
                f"{YELLOW}[BROADCAST] "
                f"unavailable: {exc}. "
                f"Falling back to primary agent."
                f"{RESET}"
            )
...
def ask_seminar_agent(
    prompt: str,
    history: list[dict] | None = None,
    voice: bool = False,
) -> str:
    """Run a Seminar agent with no tool or execution capability."""
    ...
    try:
        ...
        def _worker_call() -> str:
            prompt = (
                f"AGENT WORKER MODE\n"
                f"Project Goal: {goal}\n"
                f"Execute Subtask {task_id}: "
                f"{desc}\n\n"
                "Complete the task using available "
                "tools. Be precise and thorough."
            )
        ...
        def _critic_call(
            ...
            prompt = (
                ...
                f"Worker Output:\n"
                f"{worker_reply}\n\n"
                "Verify if the task was completed "
                "correctly. Reply with exactly "
        ...
        if "VERIFIED" in critic_reply.upper():
            ...
            state_manager.update_subtask(
                ...
                notes=(
                    "Execution accepted; "
                    "LLM critic advisory only. "
                    "No epistemic proof established."
                ),
                verification=(
                    "CRITIC_ADVISORY_ONLY"
                ),
            )

            return (
                f"Subtask {task_id} completed. "
                "Critic marked it VERIFIED as an "
                "advisory execution result; "
        ...
        print(
            f"{YELLOW}[Agent] "
            f"Critic rejected task {task_id}. "
        ...
        retry_critic_reply = _critic_call(
            retry_worker_reply
        )
        ...
        if "VERIFIED" in retry_critic_reply.upper():
            ...
            return (
                f"Subtask {task_id} completed "
                f"on retry. Critic verification "
                f"is advisory only; no epistemic "
                f"proof was established."
            )
